...
const AestheticCircle: React.FC = () => {
  const data = { aesthetics: aestheticData };
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-10 mb-14 items-start">
          <div className="lg:col-span-7 bg-white rounded-[3rem] p-10 md:p-14 border border-dark-brown/5 shadow-sm">
            <div className="flex items-center gap-6 mb-8">
              ...
              <div>
                <p className="text-[10px] uppercase tracking-widest font-black text-dark-brown/40 mb-1">{assessmentLabel}</p>
                <h2 className="font-display text-4xl text-dark-brown">{mainService.title}</h2>
              </div>
            </div>
            ...
            <div className="space-y-6 text-dark-brown/80">
              {assessmentHighlights?.map((highlight: any, i: number) => (
                <div key={i} className="flex gap-4 items-start">
                  ...
                  <p>
                    <span className="font-black uppercase tracking-widest text-xs block mb-1">{highlight.title}</span>
          ...
          <div className="lg:col-span-5 bg-bronze-dark text-cream rounded-[3rem] p-10 md:p-14 shadow-2xl relative overflow-hidden flex flex-col justify-between min-h-[400px]">
            ...
            <div>
              <h3 className="font-display text-4xl mb-6 relative z-10 leading-tight">{synergyCard?.title}</h3>
            ...
            <div className="flex flex-col gap-4 relative z-10">
              <div className="rounded-2xl border border-white/10 bg-white/5 p-6 hover:bg-white/10 transition-colors">
                <p className="text-[10px] uppercase tracking-widest font-black text-accent-orange mb-2">{synergyCard?.scienceLabel}</p>
                <p className="font-bold text-cream">{synergyCard?.scienceTitle}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-6 hover:bg-white/10 transition-colors">
                <p className="text-[10px] uppercase tracking-widest font-black text-accent-orange mb-2">{synergyCard?.natureLabel}</p>
                <p className="font-bold text-cream">{synergyCard?.natureTitle}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-14">
          <div className="bg-white rounded-[3rem] p-10 md:p-14 border border-dark-brown/5 shadow-sm">
            <div className="flex items-center gap-6 mb-8">
              ...
              <div>
                <p className="text-[10px] uppercase tracking-widest font-black text-dark-brown/40 mb-1">{scienceSection?.tagline}</p>
                <h3 className="font-display text-3xl text-dark-brown">{scienceSection?.title}</h3>
              </div>
            </div>
              ...
              {scienceSection?.items?.map((item: any, i: number) => (
                <div key={i} className="flex gap-4 items-start">
                  ...
                  <p className="text-sm leading-relaxed">
                    <span className="font-bold text-dark-brown">{item.title}:</span>{" "}
          ...
          <div className="bg-white rounded-[3rem] p-10 md:p-14 border border-dark-brown/5 shadow-sm">
            <div className="flex items-center gap-6 mb-8">
              ...
              <div>
                <p className="text-[10px] uppercase tracking-widest font-black text-dark-brown/40 mb-1">{natureSection?.tagline}</p>
                <h3 className="font-display text-3xl text-dark-brown">{natureSection?.title}</h3>
              </div>
            </div>
            <div className="space-y-6 text-dark-brown/70">
        ...
        <section className="bg-bronze-dark text-cream rounded-[4rem] p-10 md:p-20 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            <div className="lg:col-span-7">
              <span className="uppercase tracking-widest font-black text-xs text-accent-orange mb-4 block">{packageDeal?.badge}</span>
              <h2 className="font-display text-5xl md:text-7xl mb-8 leading-tight">{packageDeal?.title}</h2>
              ...
              <div className="rounded-[2rem] border border-white/10 bg-white/5 p-8 text-cream/80">
                <p className="font-black uppercase tracking-widest text-[10px] text-accent-orange mb-3">{packageDeal?.bundleLabel}</p>
                <p className="text-sm leading-relaxed">
